...
**Karga** is a robust, event-driven microservices architecture designed for a scalable ride-hailing application (similar to Martı Tag/Uber/Lyft). It is built with **Spring Boot 3**, **Apache Kafka (KRaft mode)**, **Redis (Geo-spatial)**, and **PostgreSQL**, fully containerized with **Docker**.
...
This project uses **Docker Secrets** to manage sensitive data. You **must** create the secret files before starting the containers.
...
## 🧪 Testing
The project uses Testcontainers for integration testing. This ensures tests run against real instances of PostgreSQL, Redis, and Kafka inside Docker containers.
```bash
./mvnw clean verify
```
